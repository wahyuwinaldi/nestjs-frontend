...
export const PRESET_OPTIONS = Object.keys(THEME_PRESETS);
...
/**
 * @param {Partial<{primary:string, surface:string|null, preset:string, menuMode:string}>|null|undefined} input
 */
export function normalizeTheme(input) {
    const primaryNames = new Set(PRIMARY_COLORS.map((c) => c.name));
    const surfaceNames = new Set(SURFACE_COLORS.map((s) => s.name));
    const primary = primaryNames.has(input?.primary) ? input.primary : DEFAULT_UI_THEME.primary;
    const surface = surfaceNames.has(input?.surface) ? input.surface : null;
    const preset = THEME_PRESETS[input?.preset] ? input.preset : DEFAULT_UI_THEME.preset;
    const menuMode = input?.menuMode === 'overlay' ? 'overlay' : 'static';
    return { primary, surface, preset, menuMode };
}

/**
 * @param {string} primaryName
 */
export function getPrimaryPresetExt(primaryName) {
    const color = PRIMARY_COLORS.find((c) => c.name === primaryName) || PRIMARY_COLORS.find((c) => c.name === DEFAULT_UI_THEME.primary);
...
/**
 * Apply primary and/or surface palette without changing the base preset.
 * @param {'primary'|'surface'} type
 * @param {{name:string, palette?:Record<string,string>}} color
 * @param {string} [primaryName]
 */
export function applyThemePart(type, color, primaryName) {
    if (type === 'primary') {
        updatePreset(getPrimaryPresetExt(primaryName || color.name));
...
/**
 * Full theme apply (preset + primary + surface).
 * @param {{primary:string, surface:string|null, preset:string}} theme
 */
export function applyFullTheme(theme) {
    const normalized = normalizeTheme(theme);
    const presetValue = THEME_PRESETS[normalized.preset] || THEME_PRESETS.Aura;
    const surfacePalette = SURFACE_COLORS.find((s) => s.name === normalized.surface)?.palette;
    $t().preset(presetValue).preset(getPrimaryPresetExt(normalized.primary)).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
    return normalized;
}

/**
 * Resolve PrimeVue preset module by name.
 * @param {string} presetName
 */
export function resolvePreset(presetName) {
    return THEME_PRESETS[presetName] || THEME_PRESETS.Aura;
}
